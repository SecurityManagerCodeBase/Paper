...
We characterize each application along one of four types, depending on how they
...
Of the four possible scenarios outlined in
Section~\ref{sec:Security-Manager-Study}, Type 2 applications are the most
interesting, because they change pre-configured and running security managers.
We therefore focuse on these applications therefore in our discussion below. 
However, a few Type 1 applications are discussed due to
the interesting insights they provide into benign sandbox interactions. We
discuss application that use the sandbox for purposes that are not security
related in Section \ref{sub:Non-security-uses-of} and applications that use the
sandbox for its intended security purposes in Section
...
In short, the strong mitigation is the only proposed mitigation that can be
