...
\subsection{Validating the tool with web applications}
\begin{table*}
...
\midrule
ArgoUML& UML Diagramming Tool& -having problems getting this number-& 1244& 1/11/15 \tabularnewline
...
For each of the 4 applications listed in Table \ref{tab:validation-programs}, the applications contained test cases or sample workloads in their source distribution.  We first confirmed that the application's test cases pass without a security manager set.  Then we set a security manager that printed all the permissions required by the application.  We then put each unique permissions into a policy file, and granted folder access instead of specific file access.  We then confirmed that the test cases pass when a security manager was set with the required permissions.  Finally, we set our tool and ran the application with our tool and the security manager set.  All test cases passed for all applications when using our tool.
