...
manager, which shows developers are using the sandbox for unintended purposes.
...
\emph{change a set security manager} over the course of execution.
...
in Java at the moment.  However, it is problematic for other applications that desire 
...
to all applications which run inside the Java web sandbox.
...
execution enough that our monitors are not suitable for adoption with the current 
JVMTI implementation. %on their 
...
\texttt{System}. While this rule catches many exploits, a few 
would still be possible.
For example, type confusion attacks that masquerade
as a privileged class loader will not be detected by our agent.
...
downloads.  These results show our rules would stop the vast majority of 
current exploits.
