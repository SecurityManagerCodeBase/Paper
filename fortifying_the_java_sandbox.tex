...
Costello& GUI for GUI Unit Tests & Not Open Source & 30 min. random GUI interaction and a provided example & 5/09/12 \tabularnewline
...
To validate the backwards-compatibility of our rules, we attached our monitors to the execution of any test cases provided with each application in table \ref{tab:validation-programs} or ran sample workloads when tests were unavailable.  After confirming that the tests or sample workloads worked without a security manager, we developed a reasonable security policy for each by using a custom security manager to print out unique permissions during their execution.  Finally, we ran each case a third time using our policy and the standard Java security manager with our monitors attached and enforcing the rules. The rules did not break any unit tests or sample workloads.

%Zack - I'm not sure exactly what is needed here so I'm trying to give whoever is writing it, all the information.
To test Costello and CrossFTP, we first created an automated GUI tester that randomly clicked and typed into different places in the GUI.  The code for this random GUI tester is in the GitHub respository.  We first set a policy for the applications that allowed all permissions.  We then confirmed that our tool did not prevent Costello and CrossFTP handling random GUI interactions.  We also ran a sample workload for each application.  For Costello, we ran one of the provided GUI tests and confirmed that the tool did not effect the Costello.  For CrossFTP, we transmitted a text file from the virtual machine we were testing on to the host computer using FTP.  All tests were completed successfully.
