...
applications~\cite{gong1997going,gong2003inside}.  Finally, Java \js{I don't think contains is the right word. Perhaps "Java has contained" or "Java has been inflicted with" Michael what do you think?}contains
...
executing their payloads, defects in the JRE implementation of these security mechanisms can permit
...
and interact with the security manager.  Thus, we required a variety of applications, 
ranging from heavily-used applications to less well known applications. We looked 
into using applets in our dataset, but due to Java's restrictions on applets,
we found benign applets did not interact with the security manager. Instead, 
we decided to use
the QualitasCorpus (QC) version 20130901~\cite{QualitasCorpus:APSEC:2010}, a 
collection of popular open source Java applications created for empirical studies.
While QC contains 112 applications, we found only 24 applications interacted with 
the security manager.  These 24 applications provided a set of heavily 
developed applications, but we also wanted to include applications which did not 
meet the requirement to be included in QC.  We included these other applications by 
adding 12 applications from Github that interact with the security manager, bringing
the total dataset size to 36 applications.


We identified which applications in the full QC set use the security
...
in test cases, including changing the security manager (which 
