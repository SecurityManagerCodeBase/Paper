...
complexity. These applications provide insights into cases that are currently hard for Java developers to handle without abusing security features. The abuse of security features increases the difficultly of implementing effective mitigations for attacks against those features by increasing the odds of backwards compatibility issues.
...
Benchmarks~\ref{DaCapo:paper}~(\texttt{avrora, batik, eclipse, fop, h2, jython, luindex,
lusearch, pmd, sunflow, tomcat, tradebeans, tradesoap, xalan}). Dacapo was used to validate that our rules do not break representative desktop applications, including a subset of our study's set and additional applications that help ensure the rules are not overly influenced by the study's set. For all
applications but batik, we set a security manager that granted all
permissions and attached our monitors to executions of the application.\todo{Does dacapo come with
  indicative workloads?  I genuinely don't remember.} Batik was the only
exception because it terminates if a security manager is set before starting the application: It needs to set a security manager itself. We used our monitors on Batik as set and used its
own security manager. Our rules did not break any application in Dacapo.
