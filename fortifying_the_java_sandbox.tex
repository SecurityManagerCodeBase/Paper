...
\todo{I think this train of thought needs one more observation, starting with
  ...
  should come after the ``because'', however, hence this todo.// MM: Added words.} It was not intended that the sandbox would be used to achieve these use cases, which effectively limit the set of potential exploit mitigations that are backwards-compatible with benign applications. Still, applications use the sandbox to achieve objectives that do not improve the security of the application because Java does not provide a better mechanisms for achieving their goals. 
