...
Java provides a sandbox that enables the safe execution of untrusted code and isolates
...
There are a couple ways to maliciously disable a security manager.
...
model.  %These each eliminate one or more
...
JVM modifications without setting a self-protecting security manager.  Batik,
...
This method
requires a defenseless sandbox, since it provides a trivial mechanism for
...
we were unable to discern the rationale from the examples
...
Ant, Freemind, and Netbeans explicitly set then change the manager
during runtime, which make it impossible to stop all reconfiguring, disabling,
