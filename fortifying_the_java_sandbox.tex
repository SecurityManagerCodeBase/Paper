...
Java is a popular target for malicious attackers, % (see
%Figure~\ref{fig:most-targeted-applications}), 
...
little purpose. \todo{In sections \ref{sec:Rules-for-Fortifying} and \ref{sec:Mitigations}
...
that stores the proxy security manager. This shows two common problems with the sandbox: (1) the developers did not understand that using Java policies as a blacklist is dangerous and (2) the ability to modify the manager at runtime required the use of an insufficient work-around to try to defend against malicious changes. In this case, these problems were fatal to the security of the application.}
\end{figure}
...
Application information for programs which we tested with our tool.}

\begin{tabular}{lllll}
\toprule 
\textbf{Name} & \textbf{Purpose} & \textbf{KLOC} & \textbf{Number of Test Cases} & \textbf{Latest Commit Date}\tabularnewline
\midrule
ArgoUML& UML diagramming tool & 389 & 1244 test cases& 1/11/15 \tabularnewline
Costello& GUI for GUI unit tests & not open source & 9 provided examples & 5/09/12 \tabularnewline
CrossFTP& FTP client & not open source & 30 min. random GUI interaction and sample workload& 1/18/15 \tabularnewline
JavaOpenStreetMap & interactive map display & 343 & 406 test cases& 1/18/15 \tabularnewline
...
To validate the backwards-compatibility of our rules, we attached our monitors to the execution of any test cases provided with each application in table \ref{tab:validation-programs} or ran examples provided with the application download when tests were unavailable.  After confirming that the tests or sample workloads worked without a security manager, we developed a reasonable security policy for each by using a custom security manager to print out unique permissions during their execution.  Finally, we ran each case a third time using our policy and the standard Java security manager with our monitors attached and enforcing the rules. The rules did not break any unit tests or sample workloads.

%Zack - I'm not sure exactly what is needed here so I'm trying to give whoever is writing it, all the information.
CrossFTP was the only application without test cases or provided examples in our dataset.  To test CrossFTP, we first created an automated GUI tester that randomly clicked and typed into different places in the GUI.  The code for this random GUI tester is in the GitHub respository.  When running the random GUI interaction test, we first set a policy for the application that allowed all permissions.  We then confirmed that our tool did not prevent CrossFTP handling random, unbiased GUI interactions. We also wanted to ensure that CrossFTP was still working while using our tool.  Thus, we transmitted a text file from the virtual machine we were testing on to the host computer using FTP.  Both the random GUI interaction test and the file transmission test completed successfully.
