...
with respect to their ability to guard against ten applets in a popular exploit development and delivery framework, Metasploit
4.10.0%
...
of such programs to support our claim of generalizable and indicative results. While it would seem to make sense to use benign applets for this purpose given our motivation in the form of malicious applets, we must look elsewhere. Benign applets have no reason to interact with the sandbox as long as they are following the well-defined rules imposed on them and we could not find evidence that maintained, Internet-facing websites use applets. We constructed such a dataset by
...
\footnote{\url{http://goo.gl/dtcqTM}}
...
interactions. The static analysis tool is a 
...
\todo{if the defenseless vs self-protecting observation/distinction is
  important/a contribution, I think it should be emphasized more
  strongly/directly in this subsection.}
...
\someone{We say ``most'', here.  What do the rest of them do?}
...
complexity.  \todo{These applications are interesting because...CLG thinks we
  need this sentence}.
...
\zfc{the end of this sentence contradicts what has been said in the paper; 
multiple times in the paper we have statements that say "if we can change 
the security manager then the security manager is defenseless", for example
"Both of these cases require a defenseless security manager, otherwise the
application would not be able to change the current security manager"; as 
written, this technique requires changes the security manager; thus the 
technique is not possible without defenseless security manager.}
...
\todo{CLG thinks that this subsection needs both more structure and more
  explicit observations of the insight that we're deriving from our
  observations.  It's describing a bunch of cases that are, on their own,
  relatively interesting.   However, it could do a substantially better job
  clarifying the high-level lessons or important observations we think these
  cases support.  Since I'm not quite sure what those lessons should be, I'm
  leaving a long todo in here and entrusting the task to one of the experts in the java
  sandbox...}
...
\todo{Possibly a bit of summary/discussion?  Depends on how/whether we address
  my concern at the top of the subsection regarding structure of this content.}
...
allow access via the \texttt{accessClassInPackage} permission.\todo{Question:
  wouldn't that permission result in a defenseless manager?}

% CLG moved this text from above, because it seemed really repetitive; not sure
% if any of the material it covers is critical/should be included here?
...
manager. Any further changes to the self-protecting manager are logged.\todo{you
  mean even in the presence of a defenseless manager?}
...
\todo{Content question: does this mean that we detect it the next time the
  security manager is checked, and not at the point that it is modified?  If so,
I think that could be clearer; if not, I'm confused.}
...
by the modification event.\todo{Do we do anything about it?}
...
To evaluate evaluate our rules' ability to 
block sandbox-disabling exploits, 
we ran ten Java 7 exploits for the browser from Metasploit 4.10.0 on 64-bit
\todo{half-sentence reminding the reader of what metasploit is.}
...
%Java exceptions we did not debug. 
...
stops all of the exploits on its own.\todo{then why have the other one?  IIRC
  the ansewr is ``because SM is hella slow and PE is not'', but we haven't said
  that at any point, and I'm pretty sure we don't.  Without that explanation,
  the reader is left wondering why we have the PE rule at all.}  All  
...
\todo{``These results are interesting because...}
...
table~\ref{tab:validation-programs}. \todo{More detail needed in step two.  Why
  did we develop a separate set of validation programs instead of just using the
  original programs?  The answer only helps us in terms of the validity of the
  study.  It also mitigates the threat that we've overfitted our conclusions to
  the original dataset, which we might mention in the next section.}  ArgoUML, JavaOpenStreetMap, and mucommand
...
server as a sample workload. \todo{Add footnote linking to anonymized version of
  fuzzer source code:
  https://github.com/SecurityManagerCodeBase/GUITest/blob/master/RunGUITest.java
}

In each case we confirmed that the tests, sample workloads, or fuzzed executions
worked without a security manager. We then developed reasonable security
policies for each by using a custom security manager to print out unique
permissions during their executions.\todo{the preceding sentence doesn't make
  much sense on its own, I'm afraid.  Uh, what?} Finally, we ran each case a third time
...
We then confirmed that our tool could run around the Dacapo
Benchmarks~\ref{DaCapo:paper}~(avrora, batik, eclipse, fop, h2, jython, luindex,
lusearch, pmd, sunflow, tomcat, tradebeans, tradesoap, xalan). \todo{A: typeset
  all program names.  That applies above, too.  B: why Dacapo on top of the others?} For all
applications but batik, we set a security manager that allowed for all
permissions and ran our tool around the application.\todo{Does dacapo come with
  indicative workloads?  I genuinely don't remember.}  Batik was the only
exception, since the application was written to terminate if a security manager
was set before starting the application, as batik wanted to set a security
manager itself.  Thus, we set our tool around batik and allowed batik to set its
own security manager.  All applications successfully completed when using our
tool.
...
\todo{...I'm possibly being stupid, but the connection between security
  libraries here and our work is not obvious/could be more explicit.  Half
  sentence more?}
...
gap. \todo{this is a pretty important motivating sentence, to the point that I
  wonder if we can't put it somewhere more prominent...conclusion, maybe?}
